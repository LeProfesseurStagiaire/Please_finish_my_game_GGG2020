...
func _on_Area2D_area_entered(area):
	if area.is_in_group("enemi"):
		...
		#use call_deferred (call methode befor all) beacause queue_free is faster than addchild
		call_deferred("explode_func")
		die()
...
func explode_func():
	get_tree().get_current_scene().call_deferred("add_child",explode)